...
/// <summary>NFX006: the everyday calls need a NetworkObject in the scene of the class they are used on.</summary>
public class NetworkedNodeAnalyzerTests
{
    ...
    private const string NetworkObjectScript = "res://addons/netfox-net/NetworkObject.cs";
    ...
    private const string UsesItself = "using Netfox; namespace Game; public partial class Thing : Godot.RigidBody3D { public bool Mine() => this.Authority.IsLocal; }";
    ...
    public void ASceneWithoutANetworkObjectIsAnError()
    {
        ...
        Assert.Equal("NFX006", Assert.Single(diagnostics).Id);
    ...
    public void ANetworkObjectBelowTheRootDoesNotCount()
        => Assert.Equal("NFX006", Assert.Single(Run(UsesItself, ("game/Thing.tscn", SceneWithDeepNetworkObject("res://game/Thing.cs")))).Id);

    [Fact]
    public void ACallOnAnotherClassIsJudgedByThatClassesScene()
    {
        var caller = "using Netfox; namespace Game; public partial class Player : Godot.Node3D { public void Grab(Thing thing) => thing.TryClaim(); }";
        var thing = "using Netfox; namespace Game; public partial class Thing : Godot.RigidBody3D { }";
        ...
        Assert.Contains(diagnostics, d => d.Id == "NFX006" && d.GetMessage().Contains("'Thing'"));
    }
    ...
    {
        var pickup = "using Netfox; namespace Game; public abstract partial class Pickup : Godot.RigidBody3D { protected bool Grab() => this.TryClaim(); }";
        var thing = "using Netfox; namespace Game; public partial class Thing : Pickup { public bool Take() => this.TryClaim(); }";
        ...
        Assert.Equal("NFX006", Assert.Single(withoutScene).Id);
    }

    [Fact]
    public void APartialClassIsJudgedByTheFileNamedAfterIt()
    {
        var main = "using Netfox; namespace Game; public partial class Thing : Godot.RigidBody3D { public bool Mine() => this.Authority.IsLocal; }";
        var other = "using Netfox; namespace Game; public partial class Thing { private int _count; }";
    ...
    public void NetIsReportedToo()
        => Assert.Equal("NFX006", Assert.Single(Run("using Netfox; namespace Game; public partial class Thing : Godot.RigidBody3D { public object Obj() => this.Net(); }")).Id);
}
