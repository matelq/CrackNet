...
{
    /// <summary>The CrackNet stack this node uses; resolved when it enters the tree.</summary>
    ...
    private static readonly CrackNetLogger Logger = CrackNetLogger.ForCrackNet("NetworkTickrateHandshake");

    public TickrateMismatchAction MismatchAction { get; set; }
        = CrackNetSettings.Instance.TickrateMismatchAction;
        ...
        switch (MismatchAction)
        {
            case TickrateMismatchAction.Warn:
                Logger.Warning(
                    ...
                    "See cracknet/Time/Tickrate.", networkTime.Tickrate, peer, tickrate);
                break;
