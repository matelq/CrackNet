...
public class SyncedPropertiesGeneratorTests
{
    ...
    public void ATopLevelPartialTypeGeneratesWithoutDiagnostics()
        => Assert.Empty(Run("namespace Game; public partial class Crate { [CrackNet.Synced] public int Hp { get; set; } }"));

    [Theory]
    [InlineData("namespace Game; public partial class Level { public partial class Crate { [CrackNet.Synced] public int Hp { get; set; } } }")]
    [InlineData("namespace Game; public partial class Pickup<T> { [CrackNet.Synced] public int Hp { get; set; } }")]
    public void ANestedOrGenericTypeIsAnError(string source)
        ...
        => Assert.Contains(Run(source), diagnostic => diagnostic.Id == "CRN002");
}
