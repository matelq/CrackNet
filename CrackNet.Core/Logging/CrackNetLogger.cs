namespace CrackNet.Core.Logging;
...
/// Output sinks are pluggable so the core stays engine-agnostic; CrackNet.Godot wires them to GD.Print / PushWarning / PushError.
/// </summary>
public sealed class CrackNetLogger
{
    ...
    public CrackNetLogger(string module, string name)
    {
        Module = module;
        Name = name;
    }

    public static CrackNetLogger ForCrackNet(string name) => new("cracknet", name);
    public static CrackNetLogger ForExtras(string name) => new("cracknet.extras", name);
